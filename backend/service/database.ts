...
function prefillDB() {
    ...
    if (_rowCounter('status') == 0) {
        const prep_status_insert = db_conn.prepare('INSERT INTO status (status_name) VALUES (?)');
...
function updateUserUsernameById(user_id: string, new_username: string): number | Error {
    try {
...
function deleteUserById(user_id: string): number | Error {
    try {
        return db_conn.prepare('DELETE FROM users WHERE pk_user_id = :user_id').run(user_id);
    } catch (error) {
        assertIsError(error);
        return error;
    }
...
function addAction(action_name: string): number | Error {
    try {
        return db_conn.exec('INSERT INTO actions (action_name) VALUES (?)', action_name);
    } catch (error) {
        assertIsError(error);
        return error;
    }
...
function addDepartment(department_name: string, description?: string): number | Error {
    try {
...
function deleteDepartment(department_id: number) {
    try {
        return db_conn
            .prepare('DELETE FROM departments WHERE pk_department_id = :department_id')
            .run(department_id);
    } catch (error) {
        assertIsError(error);
        return error;
...
//#region Role CRUD
function addRole(role_name: string, department_id: number, description?: string): number | Error {
    try {
        if (typeof description !== 'undefined') {
            return db_conn.exec(
...
function getTicketOfUser(user_id: string) {
    return db_conn.prepare('SELECT * FROM tickets WHERE fk_author_id= ?').all(user_id);
}

function updateTicketStatus(ticket_id: string, new_status_id: number): number | Error {
    try {
        return db_conn.exec(
...
function addEventToTicket(
    ...
    try {
        ...
        if (typeof content !== 'undefined' && typeof encoded_images !== 'undefined') {
            return db_conn.exec(
